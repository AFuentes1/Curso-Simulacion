...
def ejercicio_b():
    ...
    def paso(x):
        ...
        nuevo_bit = bit0 ^ bit2 #Xor entre bit0 y bit2
        #desplazar a la derecha y agregar el nuevo bit a la izquierda
        x_nuevo = (x >> 1) | (nuevo_bit << (bits - 1)) #insertar como MSB (bit más a la izquierda)
        return x_nuevo
    ...
    while True: 
        ...
        if len(vistos) == 15:
            print("Se ha recorrido todo el espacio posible sin repetir.")
            break
        else: 
            print("No se ha recorrido todo el espacio posible.")
            break
